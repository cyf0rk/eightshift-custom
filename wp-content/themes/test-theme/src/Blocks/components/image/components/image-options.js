	...
	return (
		<>
			...
			{imageUse &&
				<>
					{showImageUrl &&
						<Responsive label={<IconLabel icon={icons.image} label={__('Image source', 'test-theme')} />}>
							{Object.keys(checkAttrResponsive('imageUrl', attributes, manifest)).map(function (keyName) {
								...
								if (point === 'Large') {
									point = '';
									pointLabel = __('Default (all screens)', 'test-theme');
								}
					...
					{showImageAlt &&
						<TextareaControl
							label={<IconLabel icon={icons.altText} label={__('Alt text', 'test-theme')} />}
							value={imageAlt}
					...
					{showImageFull &&
						<IconToggle
							icon={icons.size}
							label={__('Fill container', 'test-theme')}
							help={__('Span the full width of the container and ignore its maximum width', 'test-theme')}
							checked={imageFull}
