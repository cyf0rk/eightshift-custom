...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'test-theme')}>
			...
			<Responsive label={<IconLabel icon={icons.order} label={__('Order', 'test-theme')} />}>
				{breakpoints.map((keyName, index) => {
					...
					return (
						<Fragment key={keyName}>
							...
							/>
							{index === 0 &&
								<p className='es-decorative-text'>{__('Needs to be set on all columns!', 'test-theme')}</p>
							}
						</Fragment>
					);
				})}
			...
			<Responsive label={<IconLabel icon={icons.verticalAlign} label={__('Vertical align', 'test-theme')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
			...
			<Responsive label={<IconLabel icon={icons.horizontalAlign} label={__('Horizontal align', 'test-theme')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
