...
export const FeaturedPostsOptions = ({ attributes, setAttributes }) => {
	...
	const taxonomyOptions = useSelect((select) => {
		...
		return [
			{
				label: __('All taxonomies', 'test-theme'),
				value: null
			},
			...data.map(({ slug }) => {
				return {
	...
	return (
		<PanelBody title={__('Featured posts', 'test-theme')}>
			{postTypeOptions &&
				<CustomSelect
					label={<IconLabel icon={icons.file} label={__('Post type', 'test-theme')} />}
					value={postType}
			...
			{postTypeOptions && taxonomyOptions && taxonomyOptions?.length > 0 &&
				<CustomSelect
					label={<IconLabel icon={icons.editOptions} label={__('Filter by taxonomy', 'test-theme')} />}
					value={taxonomy}
			...
			{taxonomyOptions && taxonomy && termsOptions?.length > 0 &&
				<CustomSelect
					label={<IconLabel icon={icons.clipboard} label={sprintf(__('Show items from this %s', 'test-theme'), taxonomy.toLowerCase())} />}
			...
			{postTypeOptions && !taxonomy &&
				<CustomSelect
					label={<IconLabel icon={<BlockIcon iconName='esf-select' />}  label={sprintf(__('Show only these %ss', 'test-theme'), _.toLower(postType))} />}
					help={__('If blank, all are shown', 'test-theme')}
			...
			<Responsive label={<IconLabel icon={icons.itemPerRow} label={__('Items per row', 'test-theme')} />}>
			...
			<RangeControl
				label={<IconLabel icon={icons.totalItems} label={__('Maximum items', 'test-theme')} />}
			...
			<IconToggle
				icon={icons.none}
				label={__('Exclude current post', 'test-theme')}
				checked={featuredPostsExcludeCurrentPost}
				help={__('This options can only be used in post single pages.', 'test-theme')}
