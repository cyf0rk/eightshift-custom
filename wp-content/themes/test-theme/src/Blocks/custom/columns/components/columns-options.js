...
export const ColumnsOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Columns', 'test-theme')}>
			<HelpModal
				type="columns"
				buttonLabel={__('How to use Columns?', 'test-theme')}
				modalLabel={__('Columns', 'test-theme')}
			/>

			<br /><br />

			<Responsive label={<IconLabel icon={icons.gutter} label={__('Column spacing', 'test-theme')} />}>
			...
			<Responsive label={<IconLabel icon={icons.verticalSpacing} label={__('Row spacing', 'test-theme')} />}>
				{breakpoints.map((keyName) => {
					const point = ucfirst(keyName);
